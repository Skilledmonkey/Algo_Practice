...
var mostCommonWord = function(paragraph, banned) {
    ...
    let removePuncAndSplit = paragraph.replace(/[.,\/#!$%'?\^&\*;:{}=\-_`~()]/g," ").trim(' ').split(' ');

    removePuncAndSplit.forEach((word) => {
        word = word.toLowerCase()
        let isWordBanned = banned.includes(word)
        let isEmptySpace = word === "" || word ===  " "

        if (wordsHash.hasOwnProperty(word) && !isWordBanned && !isEmptySpace) {
            wordsHash[word]++;
        } else if (!isWordBanned && !isEmptySpace) {
            wordsHash[word] = 1;
        }
    });

    for (const key in wordsHash) {
        if (wordsHash[key] > most) {
            most = wordsHash[key]
            mostUsedWord = key;      
        }
    }
